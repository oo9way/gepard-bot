...
def generate_pdf2_view(request):
    ...
    for order in orders:
        ...
        for item in order.items.all():
            ...
            if item.pk not in inserted_items:
                items.append(
                    {
                        "id": item.pk,
                ...
                inserted_items.append(item.pk)
            else:
                old_item = next((i for i in items if i["id"] == item.pk), None)
                new_item = {
            ...
            selected_item = next((i for i in items if i["id"] == item.pk), None)
